 ...
 * @author padol
 */
@WebServlet(name = "AddIdeaServlet", urlPatterns = {"/AddIdeaServlet"})
...
public class AddIdeaServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddIdeaServlet</title>");
            out.println("</head>");
            out.println("<body>");
            ...
            Idea idea = new Idea(title, technology, team, filePath);
            Integer insertedId = null;

            try {
                IdeaDAO dao = new IdeaDAO();
                insertedId = dao.insertIdea(idea);
            } catch (Exception e) {
        e.printStackTrace();
    }
    ...
    if (insertedId != null && insertedId > 0) {
        ...
        response.sendRedirect("aerror.html");
    }
